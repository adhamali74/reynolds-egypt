...
function Field({
  ...
  return (
    <label className="block">
      <div className={fieldShellCls(focus)}>
        {as === "textarea" ? (
          <textarea
            name={name}
            required={required}
  ...
  return (
    <div ref={wrapRef} className="relative">
      <div className={fieldShellCls(open)}>
        ...
        >
          <span className={value ? "text-white" : "text-transparent"}>{displayLabel}</span>
          <motion.span
      ...
      <AnimatePresence>
        {open && (
          <motion.ul
            ...
            className="absolute left-0 right-0 top-full z-30 mt-2 max-h-64 overflow-y-auto overflow-hidden rounded-xl border border-white/10 bg-ink-900/95 p-1.5 shadow-[0_20px_60px_-10px_rgba(0,0,0,0.6)] backdrop-blur-xl custom-scroll"
          >
            {options.map((o) => {
...
function CvUpload({
    ...
    return (
      <div className="flex items-center gap-3 rounded-lg border border-magenta-500/40 bg-magenta-500/[0.06] px-4 py-3">
        ...
        <span className="flex-1 truncate text-sm text-white/80">
          <span className="text-white/45">{t("careers.form.cvSelected")} </span>
          {file.name}
        </span>
        <button
          type="button"
  ...
  return (
    <button
      ...
      className={`flex w-full flex-col items-center gap-3 rounded-lg border-2 border-dashed px-6 py-8 text-center transition-colors duration-200 ${
        dragging
          ? "border-magenta-500/70 bg-magenta-500/10"
          : "border-white/15 bg-white/[0.02] hover:border-white/30 hover:bg-white/[0.04]"
      }`}
    >
      <Upload size={24} className={dragging ? "text-magenta-400" : "text-white/35"} />
      <div>
        <p className="text-sm text-white/70">{t("careers.form.cvDrop")}</p>
        <p className="mt-1 text-[11px] text-white/35">{t("careers.form.cvHint")}</p>
      </div>
      <input
        ref={inputRef}
        type="file"
            ...
            {BENEFITS.map(({ Icon, titleKey, copyKey, accent }, i) => {
              ...
              return (
                <motion.div
                  ...
                  className={`group relative overflow-hidden rounded-3xl border border-white/10 bg-ink-800/40 p-7 backdrop-blur-xl transition-all duration-500 hover:-translate-y-1 ${glow}`}
                >
                  <div className="noise" />
                  <div
                  ...
                  />
                  <div className={`relative z-10 mb-5 inline-flex h-12 w-12 items-center justify-center rounded-2xl border border-white/10 bg-black/30 ${iconColor}`}>
                    <Icon size={22} />
                  </div>
                  <h3 className="relative z-10 mb-2 font-display text-lg font-semibold text-white">
                    {t(titleKey)}
                  </h3>
        ...
        >
          ...
          >
            ...
            <div className="mt-10 space-y-6">
              {[
                { n: "01", label: "Submit your application & CV" },
                { n: "02", label: "Our team reviews it personally" },
                { n: "03", label: "We reach out within 3 business days" },
              ].map(({ n, label }) => (
                <div key={n} className="flex items-start gap-4">
                  <span className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-white/10 bg-white/[0.03] font-mono text-[11px] text-white/50">
                    {n}
                  </span>
                  <p className="pt-1 text-sm text-white/65">{label}</p>
                </div>
              ))}
            </div>
          </motion.div>
          ...
          >
            ...
            <div className="relative z-10 grid grid-cols-1 gap-6 sm:grid-cols-2">
              ...
              <div className="mt-2 flex flex-col items-start gap-4 sm:col-span-2 sm:flex-row sm:items-center sm:justify-between">
                <p className="text-xs text-white/40">{t("careers.form.footnote")}</p>
                <motion.button
                  type="submit"
